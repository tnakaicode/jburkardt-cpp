...
template<int dim>
Problem4<dim>::Problem4():
    fe(FE_Q<dim>(2), 1),
    dof_handler(mesh)
{}

template<int dim>
void Problem4<dim>::read_inputs()
{
    ...
    while (getline(inputFile, line))
    {
        ...
        {            
            ...
            else if (line.find("xc")==0)//number of cycles in convergence study
            {
                tmp = line.substr(3);
                input.xc = atof(tmp.c_str());
            }
            else if (line.find("yc")==0)//number of cycles in convergence study
            {
                tmp = line.substr(3);
...
template<int dim>
void Problem4<dim>::setup_geometry (int cycle)
{  		
	if (cycle == 0)
	{
		mesh.clear();
...
template<int dim>
void Problem4<dim>::refine_grid(int cycle, const char *s)
{
	if (cycle == 0)
	{
		setup_geometry(0);
...
template<int dim>
void Problem4<dim>::output_results(int cycle, const char* s)
{	
...
template<int dim>
void Problem4<dim>::calculate_error(int cycle, ConvergenceTable &error_table)
{	
...
template<int dim>
void Problem4<dim>::run()
{	
	read_inputs(); 
	
	forcing_function->alpha = input.alpha;
...
template<int dim>
void Problem4<dim>::print_errors(bool convergence_study, ConvergenceTable error_table)
{
