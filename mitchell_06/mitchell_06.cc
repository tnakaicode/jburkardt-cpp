...
template<int dim>
Problem6<dim>::Problem6():
    fe(FE_Q<dim>(2), 1),
    dof_handler(mesh)
{}

template<int dim>
void Problem6<dim>::read_inputs()
{
...
template<int dim>
void Problem6<dim>::setup_geometry (int cycle)
{  		
	if (cycle == 0)
	{
		mesh.clear();
	...
	for (; cell!=endc; ++cell)
	{
		...
		for (unsigned int q_index=0; q_index<n_q_points; q_index++)
		{
			...
			{				
				...
				{
					Tensor<1,dim> grad_phi = fe_values.shape_grad(j,q_index);
					double phi_x = grad_phi[0];
					double phi_y = grad_phi[1];
					
					cell_matrix(i,j) += (input.eps*fe_values.shape_grad(i,q_index)*fe_values.shape_grad(j,q_index) 
								+ 2*phi_x*fe_values.shape_value(i, q_index) + phi_y*fe_values.shape_value(i,q_index))
...
template<int dim>
void Problem6<dim>::refine_grid(int cycle, const char* s)
{
	if (cycle == 0)
	{
		setup_geometry(0);
...
template<int dim>
void Problem6<dim>::output_results(int cycle, const char *s)
{	
...
template<int dim>
void Problem6<dim>::calculate_error(int cycle, ConvergenceTable &error_table)
{	
...
template<int dim>
void Problem6<dim>::run()
{	
	read_inputs(); 
	
	forcing_function->eps = input.eps;	
	boundary_function->eps = input.eps;	
	exact_solution->eps = input.eps;
...
template<int dim>
void Problem6<dim>::print_errors(bool convergence_study, ConvergenceTable error_table)
{
