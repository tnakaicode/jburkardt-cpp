...
template<int dim>
Problem8<dim>::Problem8():
    fe(FE_Q<dim>(2), 1),
    dof_handler(mesh)
{}

template<int dim>
void Problem8<dim>::read_inputs()
{
...
template<int dim>
void Problem8<dim>::setup_geometry (int cycle)
{  		
	if (cycle == 0)
	{
		mesh.clear();
	...
	for (; cell!=endc; ++cell)
	{
		...
		for (unsigned int q_index=0; q_index<n_q_points; q_index++)
		{
			for (unsigned int i=0; i<dofs_per_cell; i++)
			{
				...
				double r = sqrt(pow(q_point[0],2) + pow(q_point[1],2));
				
				for (unsigned int j=0; j<dofs_per_cell; j++)
				{
					cell_matrix(i,j) += (fe_values.shape_grad(i,q_index)*fe_values.shape_grad(j,q_index) 
								- 1/pow(input.a + r, 4)*fe_values.shape_value(i,q_index)*fe_values.shape_value(j,q_index))
...
template<int dim>
void Problem8<dim>::refine_grid(int cycle, const char* s)
{
	if (cycle == 0)
	{
		setup_geometry(0);
...
template<int dim>
void Problem8<dim>::output_results(int cycle, const char *s)
{	
...
template<int dim>
void Problem8<dim>::calculate_error(int cycle, ConvergenceTable &error_table)
{	
...
template<int dim>
void Problem8<dim>::print_errors(bool convergence_study, ConvergenceTable error_table)
{
