...
template<int dim>
Problem1<dim>::Problem1():
    fe(FE_Q<dim>(2), 1),
    dof_handler(mesh)
{}

template<int dim>
void Problem1<dim>::read_inputs()
{
...
template<int dim>
void Problem1<dim>::setup_geometry (int cycle)
{  		
	if (cycle == 0)
	{	
		mesh.clear();
    ...
    std::printf("Number of active cells:%i\n", mesh.n_active_cells());
  
    dof_handler.distribute_dofs(fe);  
    std::printf("Number of degrees of freedom:%i\n", dof_handler.n_dofs()); 
...
template<int dim>
void Problem1<dim>::assemble_and_solve()
{
...
template<int dim>
void Problem1<dim>::refine_grid(int cycle, const char *s)
{
	if (cycle == 0)
	{
		setup_geometry(0);
...
template<int dim>
void Problem1<dim>::output_results(int cycle, const char *s)
{	
...
template<int dim>
void Problem1<dim>::calculate_error(int cycle, ConvergenceTable &error_table)
{	
...
template<int dim>
void Problem1<dim>::run()
{	
	read_inputs(); 
	exact_solution->p = input.p;		
	forcing_function->p = input.p;
...
template<int dim>
void Problem1<dim>::print_errors(bool convergence_study, ConvergenceTable error_table)
{
