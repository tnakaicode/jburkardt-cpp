...
template<int dim>
Problem2<dim>::Problem2():
    fe(FE_Q<dim>(2), 1),
    dof_handler(mesh)
{}

template<int dim>
void Problem2<dim>::read_inputs()
{
...
template<int dim>
void Problem2<dim>::setup_geometry (int cycle)
{  		
	if (cycle == 0)
	{
		mesh.clear();
...
template<int dim>
void Problem2<dim>::assemble_and_solve()
{
...
template<int dim>
void Problem2<dim>::refine_grid(int cycle, const char *s)
{
	if (cycle == 0)
	{
		setup_geometry(0);
...
template<int dim>
void Problem2<dim>::output_results(int cycle, const char* s)
{	
...
template<int dim>
void Problem2<dim>::calculate_error(int cycle, ConvergenceTable &error_table)
{	
...
template<int dim>
void Problem2<dim>::run()
{	
	read_inputs(); 
	
	double alpha = M_PI/input.omega;
	
	boundary_function->alpha = alpha;	
	exact_solution->alpha = alpha;	
	
	printf("omega = %f -> alpha = %f\n", input.omega, alpha);
...
template<int dim>
void Problem2<dim>::print_errors(bool convergence_study, ConvergenceTable error_table)
{
