...
template<int dim>
Problem3<dim>::Problem3():
    fe(FE_Q<dim>(2), dim),
    dof_handler(mesh)
{}

template<int dim>
void Problem3<dim>::read_inputs()
{
...
template<int dim>
void Problem3<dim>::setup_geometry (int cycle)
{  		
	if (cycle == 0)
	{
		mesh.clear();
...
void Problem3<dim>::assemble()
{
	...
	for (; cell!=endc; ++cell)
	{
		...
		for (unsigned int q_index=0; q_index<n_q_points; q_index++)
		{
			...
			{
				grad_phi[k] = fe_values[displacement].gradient(k, q_index);				
			}
			
			for (unsigned int i=0; i<dofs_per_cell; i++)
			{	
				double grad_phi_i_x_1 = grad_phi[i][0][0];
			 	double grad_phi_i_y_1 = grad_phi[i][0][1];
			 	double grad_phi_i_x_2 = grad_phi[i][1][0];
			 	double grad_phi_i_y_2 = grad_phi[i][1][1];
			 			
				for (unsigned int j=0; j<dofs_per_cell; j++)
				{
					double grad_phi_j_x_1 = grad_phi[j][0][0];
					double grad_phi_j_y_1 = grad_phi[j][0][1];
					double grad_phi_j_x_2 = grad_phi[j][1][0];
					double grad_phi_j_y_2 = grad_phi[j][1][1];
					
					cell_matrix(i,j) += E*(1-pow(nu,2))*(1/(1-2*nu)*grad_phi_j_x_1*grad_phi_i_x_1
												+1/(2-2*nu)*grad_phi_j_y_1*grad_phi_i_y_1
												+1/((1-2*nu)*(2-2*nu))*grad_phi_j_x_2*grad_phi_i_y_1
												+1/(2-2*nu)*grad_phi_j_x_2*grad_phi_i_x_2 
												+1/(1-2*nu)*grad_phi_j_y_2*grad_phi_i_y_2
												+1/((1-2*nu)*(2-2*nu))*grad_phi_j_x_1*grad_phi_j_y_2)
...
template<int dim>
void Problem3<dim>::refine_grid(int cycle, const char *s)
{
	if (cycle == 0)
	{
		setup_geometry(0);
...
template<int dim>
void Problem3<dim>::output_results(int cycle, const char* s)
{	
...
template<int dim>
void Problem3<dim>::calculate_error(int cycle, ConvergenceTable &error_table)
{	
...
template<int dim>
void Problem3<dim>::run()
{	
	read_inputs(); 
	
	boundary_function->mode = input.mode;	
	exact_solution->mode = input.mode;
...
template<int dim>
void Problem3<dim>::print_errors(bool convergence_study, ConvergenceTable error_table)
{
