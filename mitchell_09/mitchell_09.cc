...
template<int dim>
Problem9<dim>::Problem9():
    fe(FE_Q<dim>(2), 1),
    dof_handler(mesh)
{}

template<int dim>
void Problem9<dim>::read_inputs()
{
    ...
    while (getline(inputFile, line))
    {
        ...
        {            
            ...
            else if (line.find("xc")==0)
            {
                tmp      = line.substr(3);
                input.xc = atof(tmp.c_str());
            }
            else if (line.find("yc")==0)
            {
                tmp      = line.substr(3);
                input.yc = atof(tmp.c_str());
            }
            else if (line.find("r0")==0)
            {
                tmp      = line.substr(3);
                input.r0 = atof(tmp.c_str());
            }
...
template<int dim>
void Problem9<dim>::setup_geometry (int cycle)
{  		
	if (cycle == 0)
	{
		mesh.clear();
...
template<int dim>
void Problem9<dim>::assemble_and_solve()
{
...
template<int dim>
void Problem9<dim>::refine_grid(int cycle, const char* s)
{
	if (cycle == 0)
	{
		setup_geometry(0);
...
template<int dim>
void Problem9<dim>::output_results(int cycle, const char *s)
{	
...
template<int dim>
void Problem9<dim>::calculate_error(int cycle, ConvergenceTable &error_table)
{	
...
template<int dim>
void Problem9<dim>::run()
{	
	read_inputs(); 
	
	forcing_function->a = input.a;	
	forcing_function->xc = input.xc;	
	forcing_function->yc = input.yc;	
	forcing_function->r0 = input.r0;	
	
	boundary_function->a = input.a;	
	boundary_function->xc = input.xc;	
	boundary_function->yc = input.yc;	
	boundary_function->r0 = input.r0;	
	
	exact_solution->a = input.a;
	exact_solution->xc = input.xc;
	exact_solution->yc = input.yc;
	exact_solution->r0 = input.r0;
...
template<int dim>
void Problem9<dim>::print_errors(bool convergence_study, ConvergenceTable error_table)
{
